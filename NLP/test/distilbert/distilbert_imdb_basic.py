...
start = time()
pipe = pipeline("sentiment-analysis", model="huggingface-course/distilbert-base-uncased-finetuned-imdb", framework="tf", device=0)     # devices -1 : CPU, 0 : GPU
end = time()
...
# Save result csv 
result.to_csv('./result_distilbert_imdb_batch_size_n.csv', index=False)
