...
start = time()
pipe = pipeline(model="cardiffnlp/twitter-xlm-roberta-base-sentiment", framework="tf", device=0)     # devices -1 : CPU, 0 : GPU
end = time()
...
start = time()
dataset = datasets.load_dataset("tweet_eval", 'sentiment', split='test')
end = time()
...
inputs = dataset[:len(dataset)]["text"]
...
for batch_size in [1, 2]:
	...
	labeling = {'Positive': 2, 'Neutral': 1, 'Negative' : 0}	
...
# Save result csv 
result.to_csv('./result_xlm_roberta_tweet_eval_batch_size_n.csv', index=False)
