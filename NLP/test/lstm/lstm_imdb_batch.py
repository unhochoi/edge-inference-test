...
def inference(batch_size):	
  ...
  # 데이터 1개당 추론 시간 = 전체 추론 시간 / 데이터 개수
  # print('inference_time(avg) =', inference_time / len(X_test))
  # 모든 시간을 고려했을 때, 1초당 데이터 처리 개수 = 데이터개수 / (모델 로드 시간 + 데이터셋 로드 시간 + 전체 추론 시간)
  # print('IPS =', len(X_test) / (load_model_time + load_dataset_time + inference_time))
  # inference 시간만 고려했을 때, 1초당 데이터 처리 개수 = 데이터개수 / 전체 추론 시간
  # print('IPS(inf) =', len(X_test) / inference_time)


# 모델이 저장되어있는/저장할 경로
model_name = 'lstm_imdb'
saved_model_dir=f'./{model_name}_model'
